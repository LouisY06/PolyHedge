...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (positions.length > 0) {
      fetchMarketsForPositions(positions)
        .then(setMarkets)
  ...
  const runAnalysis = async () => {
    ...
    try {
      setAnalysis(
        await analyzePortfolio(positions, { objective, beginnerMode: true }),
      );
    } catch (err: unknown) {
  ...
  const activeMarket =
    selectedMarkets.length > 0
      ? selectedMarkets[selectedMarkets.length - 1]
      : null;
          ...
          <div className="hero-card p-7 mb-7 animate-fade-in-up">
            <div className="relative z-10 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5">
              <div>
                ...
                >
                  <div
                    className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-[13px] ${
              ...
              <div className="flex items-center gap-3">
                {[
                  ...
                  >
                    ...
                    <p className="text-white font-bold text-[22px] tabular-nums mt-0.5">
                      {stat.value}
                    </p>
                  </div>
                ))}
            ...
            <section>
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={runAnalysis}
                    disabled={analysisLoading || positions.length === 0}
                  ...
                  >
                    <RefreshCw
                      size={10}
                      className={`inline mr-1 ${analysisLoading ? "animate-spin" : ""}`}
                    />
              ...
              {analysisLoading && !analysis && (
                <div className="card-static p-8 text-center">
                  <Loader2
                    size={28}
                    className="animate-spin text-blue mx-auto mb-3"
                  />
                  <p className="text-text-primary font-bold text-[15px]">
                    Analyzing your portfolio...
                  </p>
                  <p className="text-text-muted text-xs mt-1">
                    Searching Polymarket, running K2Think AI
                  </p>
                </div>
              )}
              {analysisError && (
                <div className="card-static p-5 text-center">
                  ...
                  <button
                    onClick={runAnalysis}
                    className="btn-3d btn-3d-blue bg-blue text-white font-semibold py-2 px-5 rounded-full text-[12px] cursor-pointer border-none mt-3"
                  >
                    Try again
                  </button>
                </div>
          ...
          <div className="space-y-5 lg:max-h-[calc(100vh-120px)] lg:overflow-y-auto lg:sticky lg:top-[76px] scrollbar-thin">
                  ...
                  <div className="animate-fade-in-scale">
                    ...
                    <p className="text-[12px] text-text-muted mt-1.5 font-medium">
                      ...
                      <span className="tabular-nums">
                        ${(activeMarket.volume / 1_000_000).toFixed(1)}M
                      </span>{" "}
                      Vol.
                    </p>
                  </div>
              ...
              <div className="px-5 pb-5 space-y-5">
                ...
                {activeMarket && (
                  <div className="flex gap-3">
                    {(["yes", "no"] as const).map((side) => {
                      const price =
                        side === "yes"
                          ? activeMarket.confidence
                          : 100 - activeMarket.confidence;
                      return (
                        <button
                          key={side}
                ...
                <div>
                  <div className="flex items-baseline justify-between mb-4">
                    ...
                    <span
                      className={`text-[42px] font-extrabold tracking-tight tabular-nums leading-none transition-colors duration-200 ${tradeAmount > 0 ? "text-text-primary" : "text-border"}`}
                    >
                      ${tradeAmount}
                    </span>
                  </div>
            ...
            <div className="card-static p-5 space-y-4">
              ...
              {selectedMarkets.length > 0 ? (
                ...
                <p className="text-text-muted text-[12px] text-center py-2">
                  Select events to build a hedge bundle
                </p>
              )}
